...
-- prima data faceti drop  daca aveti deja creata tabela  
   ...
   DBMS_OUTPUT.PUT_LINE(count_tabela);
...
--nu v afisa nimic pentru ca nu s-a specificat sa se afiseze ceva in procedura
begin
...
begin
  ...
  -- ATENTIE!! ;s coloanele de tip varchar2 sau char trebuie specificata si dimensiunea ca in exemplu
...
--In acest caz stim sigur ca avem doar 3 tipuri de date numeric, varchar2 si date
    FOR i IN 1 .. nr_col LOOP   
        IF descColumn(i).col_type = 2 THEN
           DBMS_SQL.DEFINE_COLUMN(v_cursor_id, i, variabila_numar);  -- avem tip numeric, nu punem precizie ca altfel da eroare de tip mismach si va merge doar cu varchar2
        ...
        ELSIF descColumn(i).col_type = 12 THEN
         ...
         ELSIF descColumn(i).col_type = 96 THEN
            DBMS_SQL.DEFINE_COLUMN(v_cursor_id, i, variabila_char, descColumn(i).col_max_len);  --avem tip varchar2/char, aici trebuie sa specificam si dimensiunea neaparat
        ...
        separator:=',';
    END LOOP;
-- HINT ->  SE POATE FOLOSI SI O VARIABILA DE TIMP VARCAHR2 care sa fie asociata cu toate coloanele si folosita apoi in COLUMN_VALUE SAU
-- SE POATE FOLOSI O COLECTIE DE VARCHAR2-uri ALE CAREI ELEMENTE LE ASOCIEM LA COLOANE...ASTA CA SA NU MAI FOLOSITI VRIABILE SPECIFICE PT FIECARE TIP IN PARTE
